...
long long runTimeSort = 0;

void sorting::startSorting(int sortType, std::vector<int>& numbers) {
    QThread::msleep(300);
    qDebug() << "start: ";

    auto startTime = chrono::high_resolution_clock::now();
    executeSort(sortType, numbers);
    auto endTime = chrono::high_resolution_clock::now();

    chrono::duration<double, std::micro> duration = endTime - startTime;

    runTimeSort = duration.count();
    emit sortingCompleted(numbers, runTimeSort);
    qDebug() << "end: ";
}


void sorting::executeSort(int numberOfSort, std::vector<int>& theOriginVector) {
...
// Sap xep noi bot
void sorting::bubbleSort(vector<int>& vectoInt) {
    int n = vectoInt.size();
    for (int i = 0; i != n; ++i) {
...
// Sap xep chen
void sorting::insertionSort(vector<int>& vectoInt) {
    int n = vectoInt.size();
    for (int i = 1; i < n; i++) {
        int key = vectoInt[i];
        int j = i - 1;
...
//Tron
void sorting::merge(vector<int>& vectoInt, int prev, int mid, int after)
{
    if(mid == after){
...
// Sap xep tron
void sorting::mergeSort(vector<int>& vectoInt, int prev, int after)
{
    if(prev < after){
...
// Sap xep chon
void sorting::selectionSort(vector<int>& vectoInt) {
    int n = vectoInt.size();
    for(int i = 0; i < n; i++){
        int minIdx = i;
        for(int j = i + 1; j < n; j++){
...
//sap xep nhanh
void sorting::quickSort(vector<int>& vectoInt, int prev, int after){
    if(prev < after){
        srand(time(0));
...
//sap xep doi cho
void sorting::interchangeSort(vector<int>& vectoInt)
{
    int n = vectoInt.size();
    for(int i = 0; i < n; i++){
...
//vun
void sorting::heapify(vector<int>& vectoInt, int i, int lastHeap)
{
    int L = 2 * i + 1;
    int R = 2 * i + 2;
    int max = i;
...
//day dung cay
void sorting::buildHeap(vector<int>& vectoInt)
{
    int n = vectoInt.size();
...
//sap xep vun dong
void sorting::heapSort(vector<int>& vectoInt)
{
    buildHeap(vectoInt);
    int n = vectoInt.size();
    for(int i = n - 1; i >= 0; i--){
...
void sorting::insertion(vector<int>& vectoInt, int idx)
{
    int n = vectoInt.size();
    for(int i = 0; i < idx; i++){
...
//sap xep vo xo
void sorting::shellSort(vector<int>& vectoInt)
{
    int n = vectoInt.size();
    int k = 10;
    while(k > 0){
...
//sap xep dem
void sorting::countingSort(vector<int>& vectoInt){
...
void sorting::countingSortByDigit(vector<int>& vectoInt, int exp, bool isNegative){
    int n = vectoInt.size();
...
//sap xep theo co so
void sorting::radixSort(vector<int>& vectoInt){
...
//sap xep thung
void sorting::bucketSort(std::vector<int>& vectoInt) {
    if (vectoInt.empty()) {
        return;
    }

    int max = *std::max_element(vectoInt.begin(), vectoInt.end());
    int min = *std::min_element(vectoInt.begin(), vectoInt.end());
    int bucketCount = std::max(1, static_cast<int>(std::sqrt(vectoInt.size())));
    int delta = (max - min) / bucketCount + 1;

    std::vector<std::vector<int>> buckets(bucketCount);

    for (int num : vectoInt) {
        int index = std::min((num - min) / delta, bucketCount - 1);
...
//sap xep flash
void sorting::flashSort(vector<int>& vectoInt){
    int n = vectoInt.size();
    if (n <= 1) return;
...
//sap xep cocktail
void sorting::shakerSort(vector<int>& vectoInt){
    int left = 0;
...
void sorting::insertionOfCube(vector<int>& vectoInt, int left, int right){
...
//Sap xep song song
void sorting::cubeSort(vector<int>& vectoInt){
    int n = vectoInt.size();
    ...
    while (len < n) {
        for (int i = 0; i < n; i += 2 * len) {
            ...
            std::merge(vectoInt.begin() + i, vectoInt.begin() + mid,
