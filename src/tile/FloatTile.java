...
public class FloatTile {
    ...
    // ================= DRAW (split into before/after player) =================
    public void drawFirst(Graphics2D g2) {
        ...
        // Draw float tiles that are above player (object before player)
        for (int row = worldTopY; row <= worldBotY; row++) {
            for (int col = worldLeftX; col <= worldRightX; col++) {
                ...
                // compute collision rect for this float tile in world coords
                int collW = gp.tileSize;
                int collH = gp.tileSize;
                int offsetX = 0;
                int offsetY = (t.normalY - 1) * gp.tileSize;
                if (t.normalX == 1 && t.normalY == 2){
                    collW = gp.tileSize; collH = gp.tileSize; offsetX = 0; offsetY = gp.tileSize;
                } else if (t.normalX == 2 && t.normalY == 2){
                    collW = 2 * gp.tileSize; collH = gp.tileSize; offsetX = 0; offsetY = gp.tileSize;
                } else if (t.normalX == 3 && t.normalY == 3){
                    collW = gp.tileSize; collH = gp.tileSize; offsetX = gp.tileSize; offsetY = 2 * gp.tileSize;
                }
                int wx = col * gp.tileSize + offsetX - (t.normalX / 2) * gp.tileSize;
                int wy = row * gp.tileSize + offsetY - (t.normalY - 1) * gp.tileSize;
                int tileBottom = wy + collH;
                int playerTop = prect.y;
                if (tileBottom > playerTop) continue; // only draw tiles above player here
                ...
                g2.drawImage(imageToDraw, drawX, drawY, gp.tileSize * t.normalX, gp.tileSize * t.normalY, null);
            }
        }
    }
    ...
    public void drawLast(Graphics2D g2){
        ...
        // Draw float tiles that are below player (object after player)
        for (int row = worldTopY; row <= worldBotY; row++) {
            for (int col = worldLeftX; col <= worldRightX; col++) {
                ...
                int tileBottom = wy + collH;
                int playerTop = prect.y;
                // draw here any tile that was not already drawn in drawFirst
                if (tileBottom <= playerTop) continue;
