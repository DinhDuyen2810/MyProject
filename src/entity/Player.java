...
public class Player extends Entity{
    ...
    public java.awt.Rectangle getCollisionRect(GamePanel gp){
        int playerRenderSize = gp.tileSize * 2;
        // collision: width = 1/2 of rendered player, height = 1/12 of rendered player
        int collW = Math.max(1, (int)Math.round(playerRenderSize * (1.0/3.0)));
        int collH = Math.max(1, (int)Math.round(playerRenderSize * (1.0/12.0)));
        // center horizontally, align to bottom vertically
        int x = (int)Math.round(worldX + playerRenderSize / 2.0 - collW / 2.0);
        int y = (int)Math.round(worldY + playerRenderSize - collH);
